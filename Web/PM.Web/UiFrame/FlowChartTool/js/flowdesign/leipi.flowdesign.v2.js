    ...
    $.fn.Flowdesign = function (options) {
        ...
        if (processData.list) {
            $.each(processData.list, function (i, row) {
                ...
                .html(row.process_name+'<br/><span class="process-flag badge ' + badge + '" style="background:#ececec"><i class="' + icon + ' icon-white"></i></span>&nbsp;')
                .mousedown(function (e) {
                    if (e.which == 3) { //右键绑定
                        _canvas.find('#leipi_active_id').val(row.id);
                        _canvas.find('#process_name').val(row.process_name);
        ...
        jsPlumb.bind("click", function (c) {
            if (confirm("你确定取消连接吗?")) {
                FlowNodeDelete(c.sourceId, c.targetId);
                jsPlumb.detach(c);
            }
        });
                ...
                $.each(toArr, function (j, n) {
                    if (n != '' && n != 0) {
                        jsPlumb.connect({
                            source: nodeId,
                            target: "window" + n
                            //,labelStyle : { cssClass:"component label" }//设置连线上显示label
                            //,label : n//id +" - "+ n
                        });
                    }
                })
                ...
                try {
                    ...
                    _canvas.find("div.process-step").each(function (i) { //生成Json字符串，发送到服务器解析
                        if ($(this).attr('id')) {
                            ...
                            aProcessData[pId]["top"] = pTop + "px";
                            aProcessData[pId]["left"] = pLeft + "px";
