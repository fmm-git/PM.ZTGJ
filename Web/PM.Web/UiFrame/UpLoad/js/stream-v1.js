...
(function () {
    ...
    SWFReference.isFlashVersionAtLeast = function (a, b, c) {
        return true;
    ...
    StreamUploader.prototype = {
        ...
        streamUpload: function (pos) {
            ...
            var blob = this.sliceFile(this.file, pos, parseInt(pos) + parseInt(this.filePiece));
            var range = "bytes " + pos + "-" + (parseInt(pos) + parseInt(blob.size)) + "/" + this.get("size");
            this.preTime = (new Date).getTime();
        ...
        uploadEventHandler: function (event) {
            ...
            switch (event.type) {
                ...
                case "abort":
                    ...
                    break;
                case "progress":
                    var total = this.get("size"), loaded = parseInt(this.bytesStart) + parseInt(event.loaded),
    ...
    Main.prototype = {
        ...
        onCancel: function (file) {
            fShowMessage("取消: " + file.name);
        },
        onStop: function () {
            fShowMessage("停止!");
        },
        onCancelAll: function (numbers) {
            fShowMessage(numbers + " 个文件取消 ");
        },
        ...
        upload: function (index, value) {
            ...
            xhr.onreadystatechange = function () {
                ...
                var responseTextObj, isCommonAttachmentList, token, server, uploadDate, uploadUser;
                try {
                    try {
                        responseTextObj = eval("(" + xhr.responseText + ")");
                        token = responseTextObj.token;
                        server = responseTextObj.server;
                        uploadDate = responseTextObj.uploadDate;
                        uploadUser = responseTextObj.uploadUser;
                    } catch (e) { }
                    if (token) {
                        if (fieldName && attachmentName) {
                            //回写datagrid
                            var url = $("#" + fieldName).length > 0 ? $("#" + fieldName).dialog("options").href : "";
                            var param = url.indexOf("?");
                            var value = param != -1 ? url.substring(param).split("&")[0].split("=")[1] : 1;
                            isCommonAttachmentList = value == 0;
                            if (isCommonAttachmentList) {
                                var datagridRow = { FileID: token, UserCode: uploadUser, LastTime: uploadDate, FileSize: vars.size, FileName: vars.name, StorageOver: "1" };
                                $("#" + attachmentName).datagrid('appendRow', datagridRow);
                            }
                            //存储令牌
                            try {
                                var _flag = { success: false, message: '' };
                                if (!isCommonAttachmentList) {
                                    _flag = window.parent.SetToken(token);
                                }
                                else {
                                    _flag.success = true;
                                }
        ...
        uploadProgress: function (a) {
            var id = a.target.get("id"), percent = Math.min(99.99, a.percentLoaded),
				totalPercent = (parseInt(this.totalUploadedSize) + parseInt(a.bytesLoaded)) * 10000 / this.totalFileSize / 100, totalPercent = Math.min(99.99, totalPercent);
            ...
            if (this.config.customered) {
                var info = {
                    ...
                    totalLoaded: parseInt(this.totalUploadedSize) + parseInt(a.bytesLoaded),
                    formatTotalLoaded: this.formatBytes(parseInt(this.totalUploadedSize) + parseInt(a.bytesLoaded)),
            ...
            var _loaded = this.formatBytes(parseInt(this.totalUploadedSize) + parseInt(bytesLoaded));
            var percent = (parseInt(this.totalUploadedSize) + parseInt(bytesLoaded)) * 10000 / this.totalFileSize / 100, percent = Math.min(99.99, percent);
            100 > percent && (percent = parseFloat(percent).toFixed(2));
            this.getNode("_stream-total-uploaded", this.totalContainerPanel).innerHTML = _loaded;
            this.getNode("stream-percent", this.totalContainerPanel).innerHTML = percent + "%";
            this.getNode("stream-process-bar", this.totalContainerPanel).innerHTML = '<span style="width: ' + percent + '%;"></span>';
        ...
        uploadComplete: function (a) {
            ...
            if (!this.uploadInfo[id]) return false;
            //var jsonData = JSON.parse(a.data);
            var info = {
                id: id,
                ...
                msg: a.data,
                //user: jsonData.user,
                fileID: a.target.config.parameters.token
            };
