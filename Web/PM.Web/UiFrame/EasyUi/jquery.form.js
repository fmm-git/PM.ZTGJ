...
(function ($) {
    ...
    function ajaxSubmit(target, options) {
        //----------处理cheackbox为未选中的情况，添加一个隐藏域文本框---------
        var cur_form = $(target);
        $(target).find(":checkbox").each(function (i) {
            if (!$(this).prop("checked")) {
                cur_form.append("<input type='hidden' value='0' name='" + $(this).attr('name') + "'></input>");
            }
        });
        //----------处理cheackbox为未选中的情况---------
        ...
        function submit(param) {
            ...
            try {
                ...
                form[0].submit();
                //去除添加的隐藏域
                var cur_forms = $(target);
                $(target).find(":checkbox").each(function (i) {
                    if (!$(this).prop("checked")) {
                        $("input[name='" + $(this).attr('name') + "']")[1].remove();
                    }
                });
            } finally {
                form.attr('action', a);
    ...
    function readonly(target, mode) {
        ...
        for (var i = 0; i < plugins.length; i++) {
            ...
            var r = t.find('.' + plugin + '-f');
            if (r.length && r[plugin]) {
                r[plugin]('readonly', mode);
            }
        }
        var rr = t.find('input[type=radio],input[type=checkbox]');
        rr.each(function () {
            var f = $(this);
            if (f) {
                if (mode) {
    ...
    //设置指定区域控件状态为非只读
    function setFormPartControlNotReadOnly(target) {
        var partCtrls = new Array();
        var t = $(target);
        var plugins = ['textbox', 'combo', 'combobox', 'combotree', 'combogrid', 'datebox', 'datetimebox', 'spinner', 'timespinner', 'numberbox', 'numberspinner', 'slider'];
        for (var i = 0; i < plugins.length; i++) {
            var plugin = plugins[i];
            var r = t.find('.easyui-' + plugin + '.' + plugin + '-f');
            if (r.length && r[plugin]) {
                r.each(function () {
                    var f = $(this);
                    if (f) {
                        $('#' + f[0].id)[plugin]('readonly', false);                        
                    }
                })
            }
        }        
    ...
    function validate(target) {
        if ($.fn.validatebox) {
            var t = $(target);
            var p = $.data(target, 'form').options.part;
            if (p && p != '') {
                t = $(target).find(p);
            }
    ...
    $.fn.form.methods = {
        ...
        readonly: function (jq, options) {
            var jqp = jq;
            var isReadonly = options;
            if (options.part && options.part != '') {
                jqp = $(jq).find(options.part);
                isReadonly = options.readonly;
            }
            return jqp.each(function () {
                readonly(this, isReadonly);
            });
        },        
        setFormPartControlNotReadOnly: function (jq, partSelector) {            
            var partControls = new Array();
            var jqp = jq;
            if (partSelector != '') {
                jqp = $(jq).find(partSelector);
            }            
            jqp.each(function () {
                setFormPartControlNotReadOnly(this);                
            })            
        },
