        ...
        defaultFilterOptions: {
            onInit: function (target) {
                ...
                input.unbind('.filter').bind('keydown.filter', function (e) {
                    ...
                    //}					
                    if (e.keyCode == 13) {//关闭了延迟自动执行过滤，现在只能手动搞回车时才执行过滤
    ...
    function init(target, filters) {
        ...
        function initCss() {
            if (!$('#datagrid-filter-style').length) {
                $('head').append(
					...
					'a.datagrid-filter-btn{display:inline-block;font-size:11px;color:#008000;line-height:1;width:22px;height:22px;margin:0;vertical-align:top;cursor:pointer;opacity:0.6;filter:alpha(opacity=60);}' +
        ...
        function createFilter(frozen) {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                //---添加回车自动执行筛选---//
                $(input[0]).bind('keydown', function (e) {                                       
                    if (e.keyCode == 13) {                             
                        var name = getPluginName(target);
                        var opts = $(target)[name]('options');
                        var rules = opts.filterRules;
                        var value = this.value;
                        var op = value == '' ? 'nofilter' : 'contains';
                        ...
                        for(var r=0;r<rules.length;r++){
                            var fInput=getFilterComponent(target, rules[r].field);
                            if (fInput[0].value == '') {
                                rules[r].op = 'nofilter';
                                rules[r].value = '';
                            }
            ...
            menu.menu({
                ...
                onClick: function (item) {
                    ...
                    if (item.name === "nofilter") {
                        btn.addClass(opts.filterBtnIconCls);
                        $(btn[0]).text("");
                    } else {
                        btn.removeClass(opts.filterBtnIconCls);
                        $(btn[0]).text(item.text);
                    }
                    //------------------------------//
                    addFilterRule(target, {
                        field: field,
        ...
        function getFilter(field) {
            ...
            var myDefalutFilter = {
                field: field,
                type: 'text',
                op: ['contains', 'beginwith', 'endwith', 'equal', 'notequal', 'less', 'lessorequal', 'greater', 'greaterorequal']
            };
            return myDefalutFilter;
            //------------------------------------//
            //return null;
        }
    }
