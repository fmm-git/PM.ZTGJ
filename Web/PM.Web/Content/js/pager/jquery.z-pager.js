	...
	var methods = {
		pageInit: function(options){
			...
			return $(this).each(function(k,v){
				...
				methods.pageData(_v, opts.current, opts.searchData);
			})
		},
		pageData: function (_v, _current, _searchData) {
			/**
			 * [opts this plug propertys]
			 * @type {Obeject}
			 */
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
				    ajaxOpts = methods.ajaxData(opts.url, _current, _searchData);
			...
			var _a = _v.find("a");
				$.each(_a,function(index,item){
					...
					_this.on("click",function(){
						...
						methods.pageData(_v, _p, o.searchData);
					})
				})
		},
		...
		ajaxData: function (_url, _current, _searchData) {
			/**
