...
$(function ($) {
    ...
    $('body').find('#make-small-nav').click(function (e) {
        ...
        if ($('#page-wrapper').is('.nav-small'))
        {
            $("#nav-col").css("overflow", "visible");
        } else
        {
            $("#nav-col").css("overflow", "auto");
        }
...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        ...
        if (row.MenuPCode == "0") {
            _html += '<li>';
            _html += '<a data-id="' + row.MenuCode + '" href="#" class="dropdown-toggle"><i class="' + row.MenuIconCls + '"></i><span>' + row.MenuName + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
            var childNodes = row.ChildNodes;
            if (childNodes.length > 0) {
                _html += MadeLab(childNodes);
            }
            _html += '</li>';
        }
    });
...
//组装多级下拉菜单
function MadeLab(childNodes)
{
    var _htmlLab = "";
    _htmlLab += '<ul class="submenu">';
    $.each(childNodes, function (i) {
        var subrow = childNodes[i];
        _htmlLab += '<li>';
        var child3row = subrow.ChildNodes;
        if (child3row.length) {
            _htmlLab += '<a data-id="' + subrow.MenuCode + '" href="#" class="dropdown-toggle"><i class="' + subrow.MenuIconCls + '"></i><span>' + subrow.MenuName + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
            _htmlLab += MadeLab(child3row);
        }
        else {
            _htmlLab += '<a class="menuItem" data-id="' + subrow.MenuCode + '" href="' + subrow.MenuUrl + '" data-index="' + subrow.Sort + '">' + subrow.MenuName + '</a>';
        }
        _htmlLab += '</li>';
    });
    _htmlLab += '</ul>';
