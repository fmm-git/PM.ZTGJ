//以下是程序引用页面的定义
var savecode = "/WebAPI/Files/OfficeSave.ashx";//保存文档程序程序
var openurl = "/WebAPI/Files/FileDownload.ashx";//打开文档url
...
//打开本地文档
function OpenLocalOffice(path) {
    TANGER_OCX_OBJ = document.getElementById("TANGER_OCX");
    TANGER_OCX_OBJ.OpenLocalFile(path);
}

//打开或新建文档
function OpenOrCreateOffice(fieldID, newofficetype) {
    //根据文档URL和newofficetype编辑文档,如果有url是编辑已有文档,如果为空根据newofficetype新建文档
    if ((typeof (fieldID) != "undefined") && (fieldID != "")) {
        try { TANGER_OCX_OBJ.BeginOpenFromURL(openurl + "?fileID=" + fieldID); } catch (err) { };
    }
    else {
...
function SaveOfficeToServer(fileName) {
    SaveToServer(fileName, "office");
}
function SaveOfficeToServerAsPDF(fileName) {
    SaveToServer(fileName, "pdf");
}

//保存office文档
function SaveToServer(fileName, saveType) {
    //var fileName = "testaaaaa";
    var result, filedot;
    //if (IsFileOpened) {
    ...
    if (saveType == "pdf") {
		...
		"name=" + fileName + "&type=" + fileType,          //与控件一起提交的参数,savetype参数为要保存的文件格式office,html,pdf。filetype参数保存文件类型
...
function intializePage() {//此代码放在body标签onload事件中
    TANGER_OCX_OBJ = document.getElementById("TANGER_OCX");
    TANGER_OCX_OBJ.AddDocTypePlugin(".pdf", "PDF.NtkoDocument", "4.0.0.5", "/SysConfig/ntkooffice/NTKOOleDocAll.cab", 51, true);
    TANGER_OCX_OBJ.AddDocTypePlugin(".tif", "TIF.NtkoDocument", "4.0.0.5", "/SysConfig/ntkooffice/NTKOOleDocAll.cab", 52);
    TANGER_OCX_OBJ.AddDocTypePlugin(".tiff", "TIF.NtkoDocument", "4.0.0.5", "/SysConfig/ntkooffice/NTKOOleDocAll.cab", 52);
}
...
function addcomments() {
    var rang = TANGER_OCX_OBJ.ActiveDocument.Application.Selection.Range;
    if (rang) {
        TANGER_OCX_OBJ.ActiveDocument.Application.Selection.Comments.Add(rang);
        TANGER_OCX_OBJ.ActiveDocument.Application.Selection.Text = "";
    }
}
...
function ToggleRibbon() {
    ...
    if ("1" == doctype) {
        ...
        //else if ("2" == doctype) {
        //    var officever = TANGER_OCX_OBJ.getOfficeVer();
        //    alert(officever);
        //    if ("12" == officever) {
        //        TANGER_OCX_OBJ.Activate(true);
        //        TANGER_OCX_OBJ.ActiveDocument.Application.ExecuteExcel4Macro('SHOW.TOOLBAR("Ribbon",False)'); //2010Ribbon全部隐藏
        //    } else if ("14" == officever) {
        //        TANGER_OCX_OBJ.ActiveDocument.CommandBars.ExecuteMso("MinimizeRibbon"); //2010Ribbon最小化
        //    }
        //    else alert("此功能只限office2007以上使用");
        //}
    else alert("此功能只限WORD使用");
}
